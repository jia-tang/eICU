...
library(vcd)
library(visreg)
df <- read.csv("~/Desktop/Data science/medical records/R/first.csv",na.strings = c(""))
...
# phenotype
df$compliance_level[df$lung_compliance>50]<-"L"
df$compliance_level[df$lung_compliance>40 & df$lung_compliance<=50]<-"I"
df$compliance_level[df$lung_compliance<=40]<-"H"
```

## Visual data exploratory analysis
```{r}
...
model<-gam(lung_compliance ~ s(pfratio),data=df,method = "REML")
summary(model)

# LOESS curves (A) and predicted marginal probabilities with 95% confidence intervals (B) for the hospital mortality versus the respiratory system compliance (Crs)
model1 = gam(mortality_num ~ s(lung_compliance), data=df, family=binomial)
...
## Odds ratio
```{r}
model4= glm(mortality_num ~ compliance_level, data=df, family=binomial)
...
## Check Linearity (didn't use)
```{r}
library(tidyverse)
library(broom)
# Select only numeric predictors
test_linearity<-function(df){
mydata<-select(df,mortality_num,max_peep,TV_IBW_calculated,bmi,age,sofatotal,pfratio)
mydata<-na.omit(mydata)
model = glm(mortality_num ~ ., data=mydata, family=binomial)
probabilities <- predict(model, type = "response")
predictors <- colnames(mydata$max_peep)
...
test_linearity(df)
# nonlinear: peep, age, bmi
# linear: pfratio, sofatotal, TV_IBW
```
